...
const editUpvote = async (req, res) => {
  try {
    ...
    if (post) {
      ...
      const vote = await Upvote.findOneAndUpdate(
        { userId: req.user._id, postId: post._id, commentId: commentId },
        { value: value }
      );
...
const deleteUpvote = async (req, res) => {
  try {
    ...
    if (post) {
      ...
      const vote = await Upvote.deleteOne({
        userId: req.user._id,
        postId: post._id,
        commentId: commentId,
      });
...
const getVoteCount = async (postId, commentId) => {
  try {
    // console.log(commentId)
    // console.log(postId)
    const votes = await Upvote.aggregate([
      { $match: { postId: postId, commentId: commentId } },
      { $group: { _id: null, voteCount: { $sum: "$value" } } },
    ]);
    // console.log(votes)
    if (votes.length < 1) {
      //empty array
