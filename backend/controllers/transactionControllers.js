...
const getTransaction = async (req, res) => {
  try {
    ...
    const transaction = await Transaction.findOne({ tutorId, studentId }).sort({
      _id: -1,
    });
    ...
    if (
      ...
      transaction.transactionCancelled
    ) {
      //return null if transaction completed
...
const cancelTransaction = async (req, res) => {
  try {
    ...
    if (
      !(
        transaction.studentId.equals(req.user._id) ||
        transaction.tutorId.equals(req.user._id)
      )
    ) {
      //check if the user is owner of the post
      return res
        .status(404)
        .json({ message: "No rights to cancel transaction!" });
    }
...
const getSelectedTransactions = async (req, res) => {
  try {
    ...
    if (selected === "Pending") {
      ...
      filter = {
        ...filter,
        $or: [{ transactionCompleted: true }, { transactionCancelled: true }],
      };
    }
    // console.log(filter)

    let transaction = await Transaction.find(filter)
      .sort({ _id: -1 })
      .populate([
        { path: "studentId", select: ["name", "tutor"] },
        { path: "tutorId", select: ["name", "tutor"] },
      ]);

    if (selected === "Completed") {
      transaction = await Promise.all(
        transaction.map(async (single) => {
          const reviewed = await getReviewByUserAndTransactionId({
            transactionId: single._id,
            reviewerId: userId,
          });
...
const modifyTransaction = async (req, res) => {
  try {
    ...
    if (action === "Accept" || action === "Reject") {
      //tutor actions (Accept / Reject service request)
      if (!transaction.tutorId.equals(req.user._id)) {
        //check if the user is tutor of the transaction
      ...
      if (action === "Accept") {
        ...
        transaction = await Transaction.findOneAndUpdate(
          { _id: transactionId },
          { tutorAccepted: false, transactionCancelled: true }
        );
      }
    } else {
      //student actions (Completed service)
      if (!transaction.studentId.equals(req.user._id)) {
        //check if the user is student of the transaction
      ...
      if (action === "Completed") {
        ...
        const tutor = await User.findById(transaction.tutorId.toString());
        await User.findByIdAndUpdate(
          transaction.tutorId.toString(),
          { tutoringCount: tutor.tutoringCount + 1 },
          { new: true }
        );
      }
    }
